// === SAMKJØRER v4.8.0-dev ===
// v4.8.0-dev: FASE 1 RASK — leser nå SYNLIGE ventende-rader rett fra planlegger-tabellen (DOM) i
//   stedet for ajax-dispatch + ett berikMedPlakat-XHR per tur + upfront-geocoding. Gruppene
//   (behandlingssted + oppmøtetid via finnSamkjoringer) dukker opp UMIDDELBART. Adresser/kart for en
//   gruppe hentes først når den åpnes. lesTabellRader() utleder behov/ledsager/tider fra raden.
//   (Fase 2 «søk flere (geo)» + per-gruppe-kart uendret — kjører fortsatt on-demand.)
(function () {
    'use strict';
    ...
    // === FASE 1 RASK: les SYNLIGE ventende-rader rett fra planlegger-tabellen (DOM) ===
    // Speiler parseDispatch sin kolonne-lesing, men på LIVE DOM — ingen ajax-dispatch, ingen
    // berikMedPlakat (ett XHR per tur), ingen geocoding. Alt finnSamkjoringer trenger for å FINNE
    // grupper utledes direkte fra raden: behov=parseBehov(behovRaa), harLedsager=L-kolonne,
    // tilNavn→til, tider fra START/OPPMTID. Presise adresser/kart hentes først når en gruppe åpnes.
    function lesTabellRader() {
        var rader = [];
        var vRader = document.querySelectorAll('tr[id^="V-"]');
        if (!vRader.length) { console.warn('[SAMKJØRER] Ingen V-rader i tabellen'); return rader; }
        var tabell = vRader[0].closest('table');
        var hodeRad = (tabell && tabell.querySelector('tr.tbh')) || document.querySelector('tr.tbh');
        var h = Array.from(hodeRad ? hodeRad.cells : []).map(function (c) { return c.textContent.toUpperCase().replace(/\s+/g, ''); });
        var idx = {
        ...
        vRader.forEach(function (tr) {
            var alleReqIds = Array.from(tr.innerHTML.matchAll(/showReq\(this,\s*(\d+)/g)).map(function (m) { return m[1]; });
            if (!alleReqIds.length) return;
            var resId = tr.getAttribute('name') || tr.id.replace(/^V-/, '');
            var txt = function (i) { return i >= 0 && tr.cells[i] ? tr.cells[i].textContent.trim().replace(/\s+/g, ' ') : ''; };
            var behovTxt = function (i) {
                if (i < 0 || !tr.cells[i]) return '';
                var imgs = tr.cells[i].querySelectorAll('img');
                if (imgs.length) return Array.from(imgs).map(function (img) { return img.alt || img.title || ''; }).join(' ');
                return tr.cells[i].textContent.trim();
            };
            var behovRaa = behovTxt(idx.behov);
            var ledsager = txt(idx.ledsager);
            var rekM = tr.innerHTML.match(/searchStatus\?nr=(\d+)/);
            rader.push({
                reqId: alleReqIds[0], resId: resId, turid: null, fane: 'ventendeOppdrag',
                ...
                startTid: txt(idx.start) || txt(idx.oppmtid),
                rmate:    txt(idx.rmate),
                ledsager: ledsager,
                behovRaa: behovRaa,
                behov:    parseBehov(behovRaa),
                harLedsager: ledsager !== '' && ledsager !== '0',
                tilNavn:  txt(idx.til) || null,
                fraNavn:  txt(idx.fra) || null,
                rekNr:    rekM ? rekM[1] : null,
                status:   txt(idx.status),
            });
        });
        console.log('[SAMKJØRER] Tabell-lesing (DOM): ' + rader.length + ' ventende-rader, ingen XHR');
        return rader;
    }
    ...
    async function kjor() {
        ...
        try {
            // FASE 1 RASK (v4.8.0-dev): les kun fra planlegger-tabellen (DOM) \u2014 ingen ajax-dispatch,
            // ingen berikMedPlakat (ett XHR per tur), ingen upfront-geocoding. Gruppene vises umiddelbart.
            // Adresser/kart for en gruppe hentes f\u00f8rst n\u00e5r operat\u00f8ren \u00c5PNER den (fase 2 / kart-tegning).
            var rader = lesTabellRader();
            console.log('[SAMKJ\u00d8RER] Fase 1 tabell-rader:', rader.length);
            if (!rader.length) {
                popup.document.body.innerHTML = '<div style="padding:24px;font-family:system-ui,sans-serif;color:#334155;line-height:1.5;">Fant ingen <b>ventende</b>-rader i planlegger-tabellen.<br>S\u00f8k opp turer i planleggeren f\u00f8rst, og kj\u00f8r Samkj\u00f8rer p\u00e5 nytt.</div>';
                return;
            }
            ...
            // FASE 1 RASK: geocode-pre-filteret er HOPPET OVER (det var nettverks-tregt \u2014 geokodet
            // hver gruppeadresse opp front). Uten geokoding blir omvei/sparing-filtrene under no-op
            // (hentGeo\u2192null), s\u00e5 alle behandlingssted-grupper vises umiddelbart. Operat\u00f8ren validerer
            // omvei via kartet n\u00e5r en gruppe \u00e5pnes (der henter vi presise adresser + rute on-demand).
