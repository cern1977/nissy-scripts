// ############################################################
// # Snippets: NissyBestillNåAuto v5.6-dev                   #
...
// # v5.6-dev: POST sesjonslogg til OUS/sesjoner.php          #
// ############################################################
...
(() => {
  ...
  // === Sesjonslogg (kun dev — POSTer til OUS-backend for historikk) ===
  const LOGG_URL = 'https://thomaswestby.no/OUS/sesjoner.php';
  const LOGG_SESSION_KEY = 'nbna.session_id';
  function getSessionId() {
    let id = null;
    try { id = localStorage.getItem(LOGG_SESSION_KEY); } catch(e) {}
    if (!id) {
      id = (crypto.randomUUID && crypto.randomUUID()) ||
           (Date.now().toString(36) + Math.random().toString(36).slice(2, 10));
      try { localStorage.setItem(LOGG_SESSION_KEY, id); } catch(e) {}
    }
    return id;
  }
  function getBrukernavn() {
    try {
      const m = document.body.innerText.match(/Innlogget som\s+([^\s<]+)/i);
      if (m) return m[1].trim();
    } catch(e) {}
    return '';
  }
  function sendLogg(payload) {
    try {
      ...
      // Bruk sendBeacon hvis tilgjengelig (overlever sideunload)
      if (navigator.sendBeacon) {
        const blob = new Blob([data], { type: 'application/json' });
        navigator.sendBeacon(LOGG_URL, blob);
      } else {
        fetch(LOGG_URL, {
          method: 'POST',
      ...
      console.warn('[Bestiller] Logg-send feilet:', e.message);
    }
  }
  ...
  el.btnClose.addEventListener('click', () => {
    ...
    sendLogg({ hendelse: 'sesjon_stopp', bestilt_total: stats.sent });
    panel.remove();
    fab.remove();
    css.remove();
  });
  ...
  window.addEventListener('beforeunload', () => {
    sendLogg({ hendelse: 'sesjon_unload', bestilt_total: stats.sent });
  });
  ...
  async function init() {
    ...
    updateStatsUI();
    setStatusUI('Stoppet');
    sendLogg({ hendelse: 'sesjon_start', bestilt_total: stats.sent });
