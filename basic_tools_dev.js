...
(function() {
    ...
    // ⚠️ EN SAMMENSLÅTT SAMKJØRING HAR INGEN PASSASJERRADER I DOM-EN (Thomas 01.09).
    //    NISSY viser den som én linje med «2▼» — antall turer og en pil — og selve turene
    //    finnes ikke før operatøren åpner gruppa. Vi fant da null rekvisisjoner og spurte
    //    «er det en pågående tur?», som sender henne på jakt etter feil ting: raden ER
    //    pågående, den er bare lukket.
    // ⚠️ PILA ER ET BILDE, IKKE ET TEGN. Cellen er «3&nbsp;<img src=images/arrowdown.gif
    //    onclick=openPopp('68097108')>» — textContent gir bare «3», så et søk etter ▼ i teksten
    //    treffer aldri. Vi kjenner igjen NISSYs egen åpne-funksjon i stedet; den er entydig.
    function erSammenslaattRad(pRad) {
        if (!pRad) return 0;
        const pil = pRad.querySelector('img[onclick*="openPopp"], img[src*="arrowdown"]');
        if (!pil) return 0;
        const celle = pil.closest('td');
        const m = celle && (celle.textContent || '').match(/(\d+)/);
    ...
    function lesPassasjererFraRad(pRad) {
        ...
        // ⚠️ X-IKONET FINNES BARE PÅ NOEN AV PASSASJERENE (Thomas 01.09). I en utvidet
        //    samkjøring hadde AUSTDAL status «Startet» — altså intet remove.gif — mens SKAPPEL
        //    sto «Akseptert» og hadde det. Vi lette etter X-ikonene FØRST og falt bare tilbake
        //    når vi ikke fant NOEN: her fant vi én, sluttet å lete, og AUSTDAL forsvant fra
        //    menyen. Operatøren kunne ikke registrere bomtur på passasjeren det gjaldt.
        //
        //    toggleManualStatusRequisition står derimot på HVER passasjer, uansett status, og
        //    bærer samme tall. Den er derfor primærkilden nå; X-ikonene er kun reserve.
        let reqIds = [...new Set([
            ...[...rh.matchAll(/toggleManualStatusRequisition\s*\(\s*[^,]+,\s*['"]?(\d+)/gi)].map(m => m[1]),
            ...[...rh.matchAll(/showReq\s*\(\s*[^,]+,\s*['"]?(\d+)\s*,/gi)].map(m => m[1]),
        ])];
        if (!reqIds.length) {
            reqIds = [...rh.matchAll(/removePaagaaendeOppdrag\('(\d+)','(\d+)'\)/g)].map(m => m[1]);
        }
    ...
    async function hentBomturnummer(ids) {
        ...
        try {
            ...
            // TRANSPORTØREN SKAL MED I MELDINGEN (Thomas 01.09: «jeg savner transportør — hvem
            // jeg skal sende avbestilling til»). Koden er ubrukelig uten en mottaker.
            // Mønsteret er Lives, som har lest feltet i drift lenge (overvaaker_live.js ~2220):
            // «Transport..?r» tåler både ø og &oslash; avhengig av hvordan svaret kom.
            const tm = h.match(/Transport..?r:<\/td>\s*<td[^>]*>\s*([^<]+)/i);
            const transportor = tm ? tm[1].replace(/\s+/g, ' ').trim() : null;
            // Avtalen skiller delte avtaler («Oslo Vanlig») der navnet alene ikke sier hvem som
            // faktisk kjører. Vises i panelet, men ikke i meldingen — sentralen kjenner sin egen.
            const am2 = h.match(/Avtale:<\/td>\s*<td[^>]*>\s*([^<]+)/i);
            const avtale = am2 ? am2[1].replace(/\s+/g, ' ').trim() : null;
            ...
            return { kode: kode, raa: m[1], opprettet: opprettet, avvik: avvik,
                     ...
                     transportor: transportor, avtale: avtale };
        } catch (e) {
    ...
    function teamsAvbestillingTekst(o) {
        ...
        // ⚠️ TRANSPORTØR SKAL IKKE I MELDINGEN (Thomas 01.09). Feltet gir ofte AVTALEN, ikke et
        //    selskap: «Norgestaxi og CT Oslo Vanlig» er en delt avtale mellom to transportører.
        //    En mottakerlinje som ikke peker på én mottaker er verre enn ingen — den ser ut som
        //    en instruks. Verdien vises i panelet, der den er kontekst for operatøren i stedet.
        // ⚠️ INITIALER ALENE — ALDRI NAVNET (Thomas 02.09). Meldingen limes inn i en Teams-tråd
        //    hos taxisentralen, og hele poenget med initialer er at navnet ikke skal dit.
        //    Malen 27.08 viste «T. T. (Test, Test)» og jeg tok med begge deler; det var en
        //    testpasient, ikke en spesifikasjon. Operatøren ser navnet i panelet uansett —
        //    øverst i tittelen — så hun kan kontrollere hvem det gjelder før hun kopierer.
        if (o.samkjort && o.navn) linjer.push('Pasient: ' + teamsInitialer(o.navn));
        return linjer.join('\n');
    }
    ...
    function visBomturPanel(hvem, nummer, status, reqId, ctx) {
        ...
        if (registrert) {
            if (ctx && ctx.bomtur && ctx.bomtur.transportor) {
                const t = document.createElement('div');
                t.style.cssText = 'margin-bottom:8px;font-size:12px;color:#cbd5e1;';
                t.innerHTML = '<span style="font-size:10px;color:#94a3b8;letter-spacing:.5px;'
                    + 'text-transform:uppercase;">Transportør</span><br>'
                    + '<b style="color:#fbbf24;">' + statusEsc(ctx.bomtur.transportor) + '</b>'
                    + (ctx.bomtur.avtale ? ' <span style="color:#94a3b8;">· '
                        + statusEsc(ctx.bomtur.avtale) + '</span>' : '');
                p.appendChild(t);
            }
    ...
    function visStatusMeny(pRad, x, y) {
        ...
        if (!pass.length) {
            const antall = erSammenslaattRad(pRad);
            alert(antall
                ? `Denne raden er en sammenslått samkjøring med ${antall} turer.\n\n`
                  + 'Åpne den med pilen (▼) først, så kan du velge hvilken pasient det gjelder.'
                : 'Fant ingen rekvisisjon i denne raden — er det en pågående tur?');
            return;
        }
    ...
    // === NISSYs EGEN KJØRETID FOR TUREN (v1.235, Thomas 04.09) ===
    // «Finner ikke reisetid, men det gjør NISSY?» — og han hadde rett: boksen
    // «Rekvisisjon … (korteste vei) · Forventet kjøretid: 1 time 41 min» er
    // ajax-dispatch?action=showcostshort&rid=<resId>, verifisert 21.08.
    //
    // ⚠️ DEN TRENGER INGEN ADRESSER. Det er hele poenget: veien over
    //    (beregnReisetidNissy) krever at fra/til lar seg STRUKTURERE i gate + husnr +
    //    postnr, og et visningsnavn som «Kirurgisk, gastro- og urologisk avdeling»
    //    gjør ikke det. Da falt vi til 60-minutters-gulvet — mens NISSY hele tiden
    //    hadde svaret liggende på rekvisisjonen.
    //
    // ⚠️ `rid` BETYR IKKE DET SAMME OVERALT i ajax-dispatch: setdidnotshow vil ha
    //    rekvisisjons-id, showcostshort og setResourceDeviation vil ha RESSURS-id.
    //    På V-rader er de samme tallet, men det er en tilfeldighet vi ikke skal bygge på.
    async function utvReisetidFraNissyTur(t) {
        const rid = t.resId || t.reqId;
        if (!rid) return null;
        try {
            const url = NISSY_ORIGIN + '/planlegging/ajax-dispatch?update=false&action=showcostshort&rid=' + encodeURIComponent(rid);
            const r = await fetch(url, { credentials: 'same-origin',
                headers: { 'X-Requested-With': 'XMLHttpRequest' } });
            if (!r.ok) return null;
            const txt = await r.text();
            // ⚠️ FORANKRE I KJØRETID-RADEN. Svaret er ~100 takstrader med FRITEKSTNAVN
            //    («Nedre Romerike km, T02», «GD 10 morgen»), og et fritt søk etter «time»
            //    i hele dokumentet ville før eller siden plukket opp et takstnavn i stedet
            //    for kjøretiden. Etiketten står i en egen celle rett foran verdien:
            //      <td …>Forventet kjøretid:</td><td class="transcostcont">1 time 41 min</td>
            //    «ø» skrives ulikt avhengig av tegnsett, så bokstaven hoppes over i mønsteret.
            const rad = txt.match(/kj\S{0,6}retid[\s\S]{0,160}?transcostcont[^>]*>([^<]+)</i);
            const felt = rad ? rad[1] : '';
            // «1 time 41 min», «2 timer», «45 min». Timer uten minutter er gyldig, så
            // minutt-leddet må være valgfritt — ellers mister vi runde svar.
            const mt = felt.match(/(\d+)\s*time[rn]?(?:\s*(\d+)\s*min)?/i);
            if (mt) return (+mt[1]) * 60 + (mt[2] ? +mt[2] : 0);
            const mm = felt.match(/(\d+)\s*min/i);
            return mm ? +mm[1] : null;
        } catch (_) { return null; }
    }
    async function utvReisetidMin(t) {
        ...
        // v1.235: NISSYs eget svar på NØYAKTIG denne turen. Prøves før ruter.php, som
        // geokoder på nytt og svarer på et litt annet spørsmål enn det NISSY stilte.
        const nissyTur = await utvReisetidFraNissyTur(t);
        if (nissyTur != null) {
            if (ER_DEV) console.log('[' + NAVN + '] utv reisetid fra NISSYs turkostnad: ' + nissyTur + ' min («' + key + '»)');
            utvLagreCache(key, nissyTur);
