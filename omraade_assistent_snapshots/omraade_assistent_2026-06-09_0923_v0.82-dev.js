...
(function () {
    ...
    async function beregnPar(pool) {
        ...
        const koord = {};
        await Promise.all(Array.from(adrSet).map(async a => { koord[a] = await hentKoord(a); }));
        const hav = (k1, k2) => haversineKm(k1[0], k1[1], k2[0], k2[1]);
    ...
    function render(data) {
        ...
        const html =
            ...
            (sisteDiag ? '<div class="trakt">🔎 ' + sisteDiag.utVent + ' ventende · ' + sisteDiag.innPaga + ' returbiler → '
                ...
                + ' → <b>' + (sisteDiag.medTreff || 0) + ' får returbil</b> (' + (sisteDiag.treff || 0) + ' treff) · <b>'
                + (sisteDiag.par || 0) + ' direkte-par</b>' + (sisteDiag.stopp ? ' · <span class="stopp">⛔ ' + esc(sisteDiag.stopp) + '</span>' : '') + '</div>' : '') +
            '<div class="grid">' +
