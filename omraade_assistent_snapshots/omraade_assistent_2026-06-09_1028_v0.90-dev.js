...
(function () {
    ...
    function aapnePopup() {
        ...
        win.document.write(
            ...
            '#skjKartDiv{flex:1;background:#0b1220}' +
    ...
    async function visSamkjorKart(par) {
        ...
        ov.innerHTML =
            ...
            '<span style="color:#94a3b8">H = hentes · 🏠 = hjem</span><button id="skjLukk">✕ Lukk</button></span></div>' +
            '<div id="skjKartDiv"></div>';
        ...
        const bounds = [];
        const tegnPasient = async (l, farge, navn) => {
            const rute = await hentRute(l.fra, l.til);
            if (rute && rute.geometri && rute.geometri.length) {
                L.polyline(rute.geometri, { color: '#0b1220', weight: 8, opacity: .65, lineCap: 'round' }).addTo(k);  // halo
                L.polyline(rute.geometri, { color: farge, weight: 4, opacity: 1, lineCap: 'round' }).addTo(k);
                rute.geometri.forEach(p => bounds.push(p));
            }
            const mark = async (adr, tekst, tip) => {
                const ll = await hentKoord(adr);
                if (!gyldigKoord(ll)) return;
                bounds.push(ll);
                L.marker(ll, { icon: L.divIcon({ className: '', iconSize: [24, 24], iconAnchor: [12, 12], html: '<div class="skjmrk" style="background:' + farge + '">' + tekst + '</div>' }) })
                    .addTo(k).bindTooltip(tip, { direction: 'top' });
            };
            await mark(l.fra, 'H', 'H · ' + esc(navn) + ' hentes: ' + esc(l.fra || ''));
            await mark(l.til, '🏠', '🏠 ' + esc(navn) + ' hjem: ' + esc(l.til || ''));
        };
        await tegnPasient(a, FARGE_A, navnA);
        await tegnPasient(b, FARGE_B, navnB);
