...
(function () {
    ...
    async function oppdaterKartLag(data) {
        ...
        win.document.getElementById('kartUt').innerHTML = utVent.map(r => kort(r, turFarge(r), (r.legs[0] || {}).til, '')).join('') || '<div class="tom">Ingen i vinduet.</div>';
        win.document.getElementById('kartInn').innerHTML = innPaga.map(r => kort(r, turFarge(r), returInfo(r).fra, returStatus(r))).join('') || '<div class="tom">Ingen.</div>';
        const ruteLag = {};
