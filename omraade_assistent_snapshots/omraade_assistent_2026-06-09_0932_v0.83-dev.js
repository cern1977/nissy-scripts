...
(function () {
    ...
    async function beregnForslag(ventende, biler, diag) {
        ...
        relevante.forEach(v => adrSet.add(v._fi.D));
        biler.forEach(p => { const e = String(p._ri.fra || '').trim(); if (e) adrSet.add(e); });
        const adrListe = Array.from(adrSet);
        diag.geokoder = adrListe.length;
        const koord = {};
        await Promise.all(adrListe.map(async a => { koord[a] = await hentKoord(a); }));
        let nKoord = 0; adrListe.forEach(a => { if (gyldigKoord(koord[a])) nKoord++; });
    ...
    async function beregnPar(pool, diag) {
        ...
        const adrSet = new Set();
        pool.forEach(v => { const d = String((v.legs[0] || {}).til || '').trim(); if (d) adrSet.add(d); });
        const adrListe = Array.from(adrSet);
        const koord = {};
        await Promise.all(adrListe.map(async a => { koord[a] = await hentKoord(a); }));
        let nKoord = 0; adrListe.forEach(a => { if (gyldigKoord(koord[a])) nKoord++; });
