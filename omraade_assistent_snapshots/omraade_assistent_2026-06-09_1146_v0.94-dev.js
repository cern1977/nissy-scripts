...
(function () {
    ...
    async function visSamkjorKart(par) {
        ...
        try {
            ...
            // ÉN kombinert rute gjennom punktene i rekkefølge = bilens faktiske kjøring.
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (!rute || !rute.geometri) continue;
                // Filtrer bort ugyldige/null-punkter — ÉN null i geometrien kræsjet Leaflet (_projectLatlngs).
                const geo = [];
                for (let g = 0; g < rute.geometri.length; g++) { const pt = rute.geometri[g]; if (Array.isArray(pt) && isFinite(pt[0]) && isFinite(pt[1])) { geo.push(pt); bounds.push(pt); } }
                if (geo.length < 2) continue;
                L.polyline(geo, { color: '#0b1220', weight: 8, opacity: .6, lineCap: 'round' }).addTo(k);  // halo
                L.polyline(geo, { color: RUTEFARGE, weight: 4, opacity: 1, lineCap: 'round' }).addTo(k);
            }
