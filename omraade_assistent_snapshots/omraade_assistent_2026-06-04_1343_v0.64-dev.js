...
(function () {
    ...
    async function hentRute(fra, til) {
        ...
        try { const ls = localStorage.getItem('omr_ru_' + key); if (ls) { const v = JSON.parse(ls); _ruteCache[key] = v; return v; } } catch (e) {}
        try {
            const r = await fetch(SERVER + '/ruter.php?fra=' + encodeURIComponent(fra) + '&til=' + encodeURIComponent(til));
            const j = await r.json();
            _ruteCache[key] = j.ok ? j : null;
            if (j.ok) { try { localStorage.setItem('omr_ru_' + key, JSON.stringify(j)); } catch (e) {} }
    ...
    async function hentKoord(adr) {
        ...
        try { const ls = localStorage.getItem('omr_kd_' + adr); if (ls) { const v = JSON.parse(ls); _koordCache[adr] = v; return v; } } catch (e) {}
        try {
            const r = await fetch(SERVER + '/geokod.php?adr=' + encodeURIComponent(adr));
            const j = await r.json();
            _koordCache[adr] = j.ok ? [j.lat, j.lon] : null;
            if (j.ok) { try { localStorage.setItem('omr_kd_' + adr, JSON.stringify(_koordCache[adr])); } catch (e) {} }
