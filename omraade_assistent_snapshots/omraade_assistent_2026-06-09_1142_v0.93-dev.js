...
(function () {
    ...
    async function visSamkjorKart(par) {
        ...
        ov.innerHTML =
            ...
            '<span style="color:#94a3b8">1→4 = bilens rekkefølge</span><button id="skjLukk">✕ Lukk</button></span></div>' +
        ...
        kandidater.forEach(seq => {
            if (!seq.every(key => gyldigKoord(P[key].ll))) return;
            let d = 0; for (let i = 0; i < 3; i++) d += hav(P[seq[i]].ll, P[seq[i + 1]].ll);
            if (d < besteLen) { besteLen = d; beste = seq; }
        });
        if (!beste) beste = kandidater[0];
        // ÉN kombinert rute gjennom punktene i rekkefølge = bilens faktiske kjøring (hent begge → lever begge).
        const RUTEFARGE = '#22c55e';
        for (let i = 0; i < 3; i++) {
            const fra = P[beste[i]].adr, til = P[beste[i + 1]].adr;
            if (!fra || !til || fra === til) continue;  // samme punkt (f.eks. felles hentested) → hopp over
            const rute = await hentRute(fra, til);
            if (rute && rute.geometri && rute.geometri.length) {
                L.polyline(rute.geometri, { color: '#0b1220', weight: 8, opacity: .6, lineCap: 'round' }).addTo(k);  // halo
                L.polyline(rute.geometri, { color: RUTEFARGE, weight: 4, opacity: 1, lineCap: 'round' }).addTo(k);
                rute.geometri.forEach(p => bounds.push(p));
            }
        }
        // Nummererte markører i besøksrekkefølge (farge per pasient: blå=A, oransje=B).
        beste.forEach((key, i) => {
            const p = P[key];
            if (!gyldigKoord(p.ll)) return;
            bounds.push(p.ll);
            L.marker(p.ll, { icon: L.divIcon({ className: '', iconSize: [26, 26], iconAnchor: [13, 13], html: '<div class="skjmrk" style="background:' + p.farge + '">' + (i + 1) + '</div>' }) })
                .addTo(k).bindTooltip((i + 1) + '. ' + esc(p.navn) + ' – ' + p.rolle + ' (' + esc(p.adr || '') + ')', { direction: 'top' });
        });
