...
(function () {
    ...
    async function beregnForslag(ventende, biler, diag) {
        ...
        diag.Dset = Dset.length; diag.Eset = Eset.length;
        if (!Dset.length || !Eset.length) { diag.stopp = 'mangler destinasjoner/returmål'; return; }
        ...
        if (!M) { diag.stopp = 'matrise-kall feilet (ORS/Google)'; return; }
        diag.matriseOk = true;
        const osloRow = M[0] || [];
        ...
        diag.treff = relevante.reduce((s, v) => s + v._treff.length, 0);
        diag.medTreff = relevante.filter(v => v._treff.length).length;
        if (!diag.treff) diag.stopp = 'tidsvindu OK, men ingen innen omvei-grense (' + OMVEI_MAKS_MIN + ' min)';
    }
    ...
    async function berik(data) {
        ...
        await beregnForslag(kandidater, matchBiler, diag);
        ...
        diag.par = utVent.reduce((s, v) => s + ((v._par || []).length ? 1 : 0), 0);
        sisteDiag = diag;
    ...
    function render(data) {
        ...
        const html =
            ...
            (sisteDiag ? '<div class="trakt">🔎 ' + sisteDiag.utVent + ' ventende · ' + sisteDiag.innPaga + ' returbiler → '
                ...
                + (sisteDiag.kandidater || 0) + ' i tidsvindu → <b>' + (sisteDiag.medTreff || 0) + ' får returbil</b> (' + (sisteDiag.treff || 0) + ' treff) · <b>'
