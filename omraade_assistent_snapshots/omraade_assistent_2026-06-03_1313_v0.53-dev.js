...
(function () {
    ...
    const VERSJON = '0.53-dev';
    // Interne GD-biler (kjører i Kongsvinger) er ikke ekte returbiler — kan skjules via checkbox.
    let skjulGD = true;
    function erGD(r) { return /^\s*GD\b/i.test(r.ressurs || ''); }
    ...
    let kartMode = false, kart = null, kartLag = null, vinduMin = 120;  // kartvisning + tidsvindu (min)
    const _ruteCache = {};
    ...
    async function hentReisetid(fra, til) {
        ...
        try { const ls = localStorage.getItem('omr_rt_' + key); if (ls) { const v = JSON.parse(ls); _rtCache[key] = v; return v; } } catch (e) {}
        try {
            ...
            if (j.ok) {
                ...
                _rtCache[key] = v;
                try { localStorage.setItem('omr_rt_' + key, JSON.stringify(v)); } catch (e) {}
                return v;
            }
        } catch (e) {}
        _rtCache[key] = null;
    ...
    function aapnePopup() {
        ...
        win.document.write(
            ...
            '.kk:hover{background:#1e293b}.kk .t{font-weight:700;color:#fbbf24}.kk .n{color:#cbd5e1}' +
    ...
    function render(data) {
        ...
        const html =
            ...
            '<div class="grid">' +
                ...
                '<div class="kol"><h2>🚐 Returbiler <label class="gdtgl"><input type="checkbox" id="skjulGD"' + (skjulGD ? ' checked' : '') + '> skjul GD</label> <span class="teller">' + innPaga.length + '</span></h2>' +
    ...
    function fargeForTur(id) { let h = 0; const s = String(id || ''); for (let i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) % 360; return 'hsl(' + h + ',75%,55%)'; }
    ...
    // Synlige ventende i tidsvinduet (hentetid fra litt før nå → nå + vinduMin).
    function synligeIVindu(utVent) {
        const naa = naaMin();
        return utVent.filter(v => {
            const t = parseTid((v.legs[0] || {}).opp || (v.legs[0] || {}).start);
            return t !== null && String((v.legs[0] || {}).til || '').trim() && t >= naa - 20 && t <= naa + vinduMin;
    ...
    async function visKart(data) {
        ...
        if (!win.document.getElementById('kartDiv')) {
            ...
            L.tileLayer('https://cache.kartverket.no/v1/wmts/1.0.0/topo/default/webmercator/{z}/{y}/{x}.png', { maxZoom: 18 }).addTo(kart);
            kartLag = L.layerGroup().addTo(kart);
    ...
    async function oppdaterKartLag(data) {
        ...
        // Panel-kort
        const kort = (r, farge, fra, til) => '<div class="kk" data-res="' + esc(r.resId) + '" style="border-left-color:' + farge + '">' +
            '<span class="t">' + esc((r.legs[0] || {}).opp || (r.legs[0] || {}).start || '–') + '</span> ' +
            '<span class="n">' + esc(r.ressurs || (r.legs[0] || {}).pnavn || '') + ' → ' + esc(stedFraAdr(til) || til || '?') + '</span></div>';
        win.document.getElementById('kartUt').innerHTML = utVent.map(r => kort(r, fargeForTur(r.resId), (r.legs[0] || {}).fra, (r.legs[0] || {}).til)).join('') || '<div class="tom">Ingen i vinduet.</div>';
        win.document.getElementById('kartInn').innerHTML = innPaga.map(r => { const ri = returInfo(r); return kort(r, fargeForTur(r.resId), ri.fra, ri.fra); }).join('') || '<div class="tom">Ingen.</div>';
        const ruteLag = {};
        // Klikk på kort → zoom til ruta
        win.document.querySelectorAll('#kartwrap .kk').forEach(el => el.onclick = () => { const lag = ruteLag[el.dataset.res]; if (lag) kart.fitBounds(lag.getBounds(), { padding: [40, 40] }); });
        ...
        const tegn = async (r, fra, til, stiplet) => {
            ...
            const farge = fargeForTur(r.resId);
            const pl = L.polyline(rute.geometri, { color: farge, weight: stiplet ? 3 : 4, opacity: 0.85, dashArray: stiplet ? '6,8' : null });
            pl.addTo(kartLag);
            L.circleMarker(rute.til, { radius: 5, color: farge, fillColor: farge, fillOpacity: 1 }).addTo(kartLag);
            ruteLag[r.resId] = pl;
        };
        utVent.forEach(r => tegn(r, (r.legs[0] || {}).fra, (r.legs[0] || {}).til, false));
        innPaga.forEach(r => { const ri = returInfo(r); const osloBen = (r.legs || []).find(l => erVaartOmraade(l.til)); tegn(r, ri.fra, osloBen ? osloBen.til : 'Oslo', true); });
    }
