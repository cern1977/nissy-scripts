...
(function () {
    ...
    let kartMode = false, kart = null, kartLag = null, casingLag = null, basisLag = null, vinduMin = 120, kartBasis = 'mørkt';
    let visReturbiler = false;  // returbiler skjult på kart som standard
    const _ruteCache = {};
    ...
    function aapnePopup() {
        ...
        win.document.write(
            ...
            '.kk{background:#0f172a;border-radius:6px;padding:5px 8px;margin-bottom:5px;border-left:5px solid #334155;font-size:11px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;gap:6px}' +
    ...
    async function visKart(data) {
        ...
        if (!win.document.getElementById('kartDiv')) {
            win.document.getElementById('rot').innerHTML =
                ...
                ' · <label><input type="checkbox" id="kartRet"' + (visReturbiler ? ' checked' : '') + '> 🚐 returbiler</label>' +
            ...
            win.document.getElementById('kartRet').onchange = e => { visReturbiler = e.target.checked; if (sisteData) oppdaterKartLag(sisteData); };
    ...
    async function oppdaterKartLag(data) {
        ...
        // Returbiler: skjult som standard — toggle styrer både panel og ruter.
        const hoyre = win.document.querySelector('.kartpanel.hoyre');
        if (hoyre) hoyre.style.display = visReturbiler ? '' : 'none';
        ...
        const kort = (r, farge, til, status) => '<div class="kk" data-res="' + esc(r.resId) + '" style="border-left-color:' + farge + '">' +
            '<span class="v"><span class="t">' + esc((r.legs[0] || {}).opp || (r.legs[0] || {}).start || '–') + '</span> ' +
            '<span class="n">' + esc(r.ressurs || (r.legs[0] || {}).pnavn || '') + ' → ' + esc(stedFraAdr(til) || til || '?') + '</span></span>' +
            (status ? '<span class="kst' + (startet(status) ? ' kjort' : '') + '">' + esc(status) + '</span>' : '') + '</div>';
        win.document.getElementById('kartUt').innerHTML = utVent.map(r => kort(r, fargeForTur(r.resId), (r.legs[0] || {}).til, '')).join('') || '<div class="tom">Ingen i vinduet.</div>';
        if (visReturbiler) {
            win.document.getElementById('kartInnN').textContent = innPaga.length;
            win.document.getElementById('kartInn').innerHTML = innPaga.map(r => kort(r, fargeForTur(r.resId), returInfo(r).fra, returStatus(r))).join('') || '<div class="tom">Ingen.</div>';
        }
        const ruteLag = {};
        ...
        // Returbiler: kun opprinnelsespunkt (markør) — fulle ruter ble spagetti.
        if (visReturbiler) innPaga.forEach(async r => {
