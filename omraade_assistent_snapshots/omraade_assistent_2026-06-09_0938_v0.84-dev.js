...
(function () {
    ...
    async function beregnForslag(ventende, biler, diag) {
        ...
        // Geokod alle destinasjoner (D = pasientens hjem) + returmål (E = der returbilen skal videre).
        // Cachet server-side 30 d + localStorage. Omvei = (Oslo→D + D→E − Oslo→E) via haversine×VEIFAKTOR
        // — IKKE ORS-matrise: den ble 100×120 og sprengte API-grensa. Luftlinje er grovt, men mer enn nok
        // som omvei-forfilter (samkjorer.js bruker samme tilnærming i produksjon).
        // rico.js saboterer Set/forEach/Array.from på NISSY-arrays → bygg adresse-lista med rene for-løkker.
        const koord = {};
        const adrListe = [];
        ...
        for (let i = 0; i < biler.length; i++) leggTilAdr(biler[i]._ri.fra);
        diag.geokoder = adrListe.length;
        const _proms = [];
        for (let i = 0; i < adrListe.length; i++) _proms.push(hentKoord(adrListe[i]));
        const _res = await Promise.all(_proms);
        let nKoord = 0;
        ...
        relevante.forEach(v => {
            ...
            const oD = hav(OSLO_KOORD, dK);   // Oslo→D
            const treff = [];
            biler.forEach(p => {
                ...
                const eK = koord[E];
                if (!gyldigKoord(eK)) return;
                const oE = hav(OSLO_KOORD, eK), dE = hav(dK, eK);     // Oslo→E, D→E
    ...
    async function beregnPar(pool, diag) {
        ...
        // rico.js saboterer Set/forEach/Array.from på NISSY-arrays → ren for-løkke for adresse-lista.
        const koord = {};
        const adrListe = [];
        for (let i = 0; i < pool.length; i++) {
            const d = String((pool[i].legs[0] || {}).til || '').trim();
            if (d && !Object.prototype.hasOwnProperty.call(koord, d)) { koord[d] = null; adrListe.push(d); }
        }
        const _proms = [];
        for (let i = 0; i < adrListe.length; i++) _proms.push(hentKoord(adrListe[i]));
        const _res = await Promise.all(_proms);
        ...
        const hav = (k1, k2) => haversineKm(k1[0], k1[1], k2[0], k2[1]);
        let pTid = 0, pKap = 0, pGeo = 0, pBeste = Infinity;
        for (let i = 0; i < pool.length; i++) {
            for (let j = i + 1; j < pool.length; j++) {
                ...
                if (!kap.ok) continue;
                pKap++;
                const kA = koord[String((a.legs[0] || {}).til || '').trim()];
                const kB = koord[String((b.legs[0] || {}).til || '').trim()];
                if (!gyldigKoord(kA) || !gyldigKoord(kB)) continue;
                pGeo++;
                // Begge hentes i Oslo, leveres med én bil: Oslo→nærmest→fjernest. omvei = (kortest Oslo-ben
                // + ben mellom de to) − lengste enkelttur.
                const oA = hav(OSLO_KOORD, kA), oB = hav(OSLO_KOORD, kB), ab = hav(kA, kB);
                const omveiKm = Math.max(0, (Math.min(oA, oB) + ab - Math.max(oA, oB)) * VEIFAKTOR);
        ...
        diag.parTidOk = pTid; diag.parKapOk = pKap; diag.parGeoParOk = pGeo; diag.parBesteOmvei = isFinite(pBeste) ? pBeste : null;
    ...
    async function berik(data) {
        ...
        await Promise.all(kand.map(async r => { const l = r.legs[0] || {}; r._rt = await hentReisetid(l.fra, l.til); }));
    ...
    function render(data) {
        ...
        const html =
            ...
            (sisteDiag ? '<div class="trakt">🔎 ' + sisteDiag.utVent + ' ventende · ' + sisteDiag.innPaga + ' returbiler → '
                ...
                + ' <span style="color:#475569">[par: pool ' + (sisteDiag.parPool || 0) + ' → tid ' + (sisteDiag.parTidOk || 0) + ' → kap ' + (sisteDiag.parKapOk || 0) + ' → geo ' + (sisteDiag.parGeoParOk || 0) + (sisteDiag.parBesteOmvei != null ? ' · beste ' + sisteDiag.parBesteOmvei + ' min' : '') + ' · geokodet ' + (sisteDiag.parGeoOk || 0) + '/' + (sisteDiag.parGeo || 0) + ']</span>'
    ...
    async function hentRute(fra, til) {
        ...
        if (ls && ls.geometri && ls.geometri.length) { _ruteCache[key] = ls; return ls; }  // bruk cache KUN hvis gyldig rute; ellers hent på nytt
        try {
