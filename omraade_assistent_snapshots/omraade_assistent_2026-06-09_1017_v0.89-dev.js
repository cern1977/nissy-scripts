...
(function () {
    ...
    function render(data) {
        ...
        const html =
            ...
            '<div class="liste">' + (parAnbefalt.length ? parAnbefalt.map(parKort).join('') : '<div class="tom">Ingen par nå.</div>') + '</div>' +
            (parAndre.length ? '<details style="margin-top:8px"><summary style="cursor:pointer;color:#94a3b8;font-size:11px">Andre mulige par (' + parAndre.length + ') – overlapper med planen over</summary><div class="liste" style="margin-top:6px;opacity:.75">' + parAndre.map(parKort).join('') + '</div></details>' : '') +
            '</div>' +
