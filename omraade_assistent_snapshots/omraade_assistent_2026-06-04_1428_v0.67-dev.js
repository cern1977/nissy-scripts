...
(function () {
    ...
    let kartMarkorer = [], kartSiste = null, kartRef = {};  // MapLibre: markører, siste tegnedata, klikk-referanser
    ...
    async function visKart(data) {
        ...
        if (!win.document.getElementById('kartDiv')) {
            ...
            // setStyle nullstiller kilder/lag → legg dem på igjen + tegn på nytt ved hver style.load
            kart.on('style.load', () => { sikreKartLag(); applyKart(); });
            ...
            win.document.getElementById('kartBasis').onchange = e => { kartBasis = e.target.value; kart.setStyle(BASISKART[kartBasis].style, { diff: false }); };
        }
    ...
    // Tegn siste beregnede kartdata (kilder + markører) — krever at stilen er lastet
    function applyKart() {
        if (!kart || !kart.isStyleLoaded() || !kartSiste) return;
        sikreKartLag();
