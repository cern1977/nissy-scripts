...
(function () {
    ...
    let _ventDumpet = false;  // TEST: dump ventende-tabellens kolonner én gang (kartlegg oppmøtetid vs reisetid)
    ...
    function parseDispatch(responseText) {
        ...
        xmlDoc.querySelectorAll('response').forEach(resp => {
            ...
            if (iFraTil >= 0) {
                ...
                if (!_ventDumpet) {
                    _ventDumpet = true;
                    console.log('[' + NAVN + '] VENTENDE-headere:', JSON.stringify(hc));
                    console.log('[' + NAVN + '] idx:', JSON.stringify(idx));
                    let _n = 0;
                    d.querySelectorAll('tbody tr[name]').forEach(tr => {
                        if (_n >= 40) return;
                        const rq = Array.from(tr.innerHTML.matchAll(/showReq\(this,\s*(\d+)/g)).map(m => m[1]);
                        if (!rq.length) return;
                        _n++;
                        const celler = [];
                        for (let i = 0; i < tr.cells.length; i++) celler.push((hc[i] || ('c' + i)) + '=' + tr.cells[i].textContent.trim());
                        console.log('[' + NAVN + '] V' + _n + ': ' + celler.join(' | '));
                    });
                }
                d.querySelectorAll('tbody tr[name]').forEach(tr => {
