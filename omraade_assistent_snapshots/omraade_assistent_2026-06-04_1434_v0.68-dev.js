// === OMRÅDE ASSISTENT v0.68-dev ===
// v0.68-dev: fiks — ruter/markører tegnet ikke (data klart før vektor-stilen var lastet → applyKart kjørte
//            mens isStyleLoaded()=false). applyKart selv-healer nå: retry på kartets 'idle'-event. Første
//            tegning via 'load', og bakgrunnsbytte kaller applyKart etter setStyle.
// v0.67-dev: kart-motor byttet Leaflet → MapLibre GL. Vektor-bakgrunn (default «Lyst grå»/positron,
//            + «Detaljert», Kartverket grå/topo som raster, Mørkt). Ruter tegnes som ett data-drevet
//            GeoJSON-lag (farge per tur via ['get','farge']); hel rute = ventende (m/casing-halo +
//            dropp-punkt), stiplet = returbil. Returbil-🚐 som MapLibre-markør. Klikk på kort → fitBounds/
//            flyTo. Rotasjon/tilt via NavigationControl. Panel-kort + matching uendret. Fallback: snapshot v0.66.
...
(function () {
    ...
    let kartMode = false, kart = null, vinduMin = 120, kartBasis = 'lyst';
    let kartMarkorer = [], kartSiste = null, kartRef = {}, _kartVenter = false;  // MapLibre: markører, siste tegnedata, klikk-ref, idle-retry-flagg
    let returVis = 'punkt';  // hvordan returbiler vises på kart: 'punkt' | 'rute'
    const _ruteCache = {};
    // Bakgrunnskart for MapLibre. style = vektor-stil-URL eller et raster-stil-objekt.
    // casing = halo-farge under rutene (lys på mørkt kart, mørk på lyst).
    const _rasterStil = (tiles, attr) => ({
        version: 8,
        sources: { r: { type: 'raster', tileSize: 256, tiles: [tiles], attribution: attr || '' } },
        layers: [{ id: 'bg', type: 'background', paint: { 'background-color': '#0b1220' } }, { id: 'r', type: 'raster', source: 'r' }],
    });
    const _kv = z => 'https://cache.kartverket.no/v1/wmts/1.0.0/' + z + '/default/webmercator/{z}/{y}/{x}.png';
    const BASISKART = {
        'lyst':      { navn: 'Lyst grå',        style: 'https://tiles.openfreemap.org/styles/positron', casing: 'rgba(15,23,42,.6)' },
        'detaljert': { navn: 'Detaljert',       style: 'https://tiles.openfreemap.org/styles/bright',   casing: 'rgba(15,23,42,.55)' },
        'gråtone':   { navn: 'Kartverket grå',  style: _rasterStil(_kv('topograatone'), '© Kartverket'), casing: 'rgba(15,23,42,.6)' },
        'topo':      { navn: 'Kartverket topo', style: _rasterStil(_kv('topo'), '© Kartverket'),         casing: 'rgba(255,255,255,.65)' },
        'mørkt':     { navn: 'Mørkt',           style: _rasterStil('https://a.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png', '© CARTO'), casing: 'rgba(255,255,255,.5)' },
    };
    ...
    function aapnePopup() {
        ...
        win.document.write(
            ...
            '.maplibregl-map,.maplibregl-canvas{background:#0b1220}' +
            '.retmark{cursor:pointer}' +
    ...
    /* ── Kartmodus (MapLibre GL + vektor/Kartverket + ORS-ruter) ── */
    ...
    function setKartMode(b) {
        kartMode = b;
        if (!b && kart) {
            kartMarkorer.forEach(m => { try { m.remove(); } catch (e) {} }); kartMarkorer = [];
            try { kart.remove(); } catch (e) {} kart = null; kartSiste = null;
        }
    ...
    async function visKart(data) {
        if (!win || win.closed) return;
        const ok = await lastMapLibre();
        const ml = win.maplibregl;
        if (!ok || !ml) { win.document.getElementById('rot').innerHTML = '<p class="tom">Kunne ikke laste kart (MapLibre).</p>'; return; }
        if (!win.document.getElementById('kartDiv')) {
            win.document.getElementById('rot').innerHTML =
                ...
                '</div>';
            kart = new ml.Map({ container: 'kartDiv', style: BASISKART[kartBasis].style, center: [10.6, 60.4], zoom: 7, attributionControl: false });
            kart.addControl(new ml.NavigationControl({ visualizePitch: true }), 'top-right');
            kart.on('load', applyKart);  // første tegning (applyKart selv-healer hvis stilen ikke er klar)
            ...
            win.document.getElementById('kartBasis').onchange = e => { kartBasis = e.target.value; kart.setStyle(BASISKART[kartBasis].style, { diff: false }); applyKart(); };
        }
        oppdaterKartLag(data);
    }
    // Tomt GeoJSON, bbox av koordinater [lon,lat], og gjeldende casing-farge
    function _tomFC() { return { type: 'FeatureCollection', features: [] }; }
    function _bbox(coords) { let a = 180, b = 90, c = -180, d = -90; coords.forEach(p => { if (p[0] < a) a = p[0]; if (p[0] > c) c = p[0]; if (p[1] < b) b = p[1]; if (p[1] > d) d = p[1]; }); return [[a, b], [c, d]]; }
    function _casing() { return (BASISKART[kartBasis] || {}).casing || 'rgba(0,0,0,.5)'; }
    // Legg på rute-/punkt-kilder + lag (idempotent — kalles ved hver style.load)
    function sikreKartLag() {
        if (!kart || kart.getSource('ruter')) return;
        kart.addSource('ruter', { type: 'geojson', data: _tomFC() });
        kart.addSource('punkter', { type: 'geojson', data: _tomFC() });
        // Casing/halo kun under hele (ikke-stiplede) ruter; stiplede returbil-ruter står uten halo.
        kart.addLayer({ id: 'rute-casing', type: 'line', source: 'ruter', filter: ['!', ['get', 'stiplet']], layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': _casing(), 'line-width': 9, 'line-opacity': 0.9 } });
        kart.addLayer({ id: 'rute-hel', type: 'line', source: 'ruter', filter: ['!', ['get', 'stiplet']], layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': ['get', 'farge'], 'line-width': 5.5 } });
        kart.addLayer({ id: 'rute-stiplet', type: 'line', source: 'ruter', filter: ['get', 'stiplet'], layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': ['get', 'farge'], 'line-width': 3.5, 'line-dasharray': [2, 2] } });
        kart.addLayer({ id: 'punkt-drop', type: 'circle', source: 'punkter', paint: { 'circle-radius': 6, 'circle-color': ['get', 'farge'], 'circle-stroke-color': '#fff', 'circle-stroke-width': 2 } });
    }
    // Tegn siste beregnede kartdata (kilder + markører). Selv-healer: er ikke stilen klar ennå
    // (data ofte klart før vektor-stilen er lastet), prøver vi igjen når kartet blir 'idle'.
    function applyKart() {
        if (!kart || !kartSiste) return;
        if (!kart.isStyleLoaded()) {
            if (!_kartVenter) { _kartVenter = true; kart.once('idle', () => { _kartVenter = false; applyKart(); }); }
            return;
        }
        sikreKartLag();
        kart.getSource('ruter').setData(kartSiste.ruterFC);
        kart.getSource('punkter').setData(kartSiste.punkterFC);
        if (kart.getLayer('rute-casing')) kart.setPaintProperty('rute-casing', 'line-color', _casing());
        kartMarkorer.forEach(m => { try { m.remove(); } catch (e) {} }); kartMarkorer = [];
        kartSiste.markorer.forEach(d => {
            const el = win.document.createElement('div'); el.className = 'retmark'; el.innerHTML = d.html; el.title = d.tip;
            kartMarkorer.push(new win.maplibregl.Marker({ element: el, anchor: 'center' }).setLngLat(d.lngLat).addTo(kart));
        });
    }
    async function oppdaterKartLag(data) {
        if (!win || win.closed || !kart) return;
        ...
        // Klikk på kort → zoom til ruta (bounds) eller punktet (returbil-markør)
        win.document.querySelectorAll('#kartwrap .kk').forEach(el => el.onclick = () => {
            const r = kartRef[el.dataset.res]; if (!r || !kart) return;
            if (r.bounds) kart.fitBounds(r.bounds, { padding: 50, duration: 600 });
            else if (r.center) kart.flyTo({ center: r.center, zoom: 11 });
        });
        // Bygg GeoJSON: ventende = hel rute + dropp-punkt, returbil = stiplet rute eller 🚐-markør
        const ruterFC = _tomFC(), punkterFC = _tomFC(), markorer = [], ref = {};
        const leggRute = async (r, fra, til, stiplet) => {
            const rute = await hentRute(fra, til);
            if (!rute || !rute.geometri) return;
            const coords = rute.geometri.map(p => [p[1], p[0]]);  // [lat,lon] → [lon,lat]
            ruterFC.features.push({ type: 'Feature', properties: { farge: turFarge(r), stiplet: !!stiplet, res: r.resId }, geometry: { type: 'LineString', coordinates: coords } });
            if (rute.til) punkterFC.features.push({ type: 'Feature', properties: { farge: turFarge(r) }, geometry: { type: 'Point', coordinates: [rute.til[1], rute.til[0]] } });
            ref[r.resId] = { bounds: _bbox(coords) };
        };
        const jobber = [];
        utVent.forEach(r => jobber.push(leggRute(r, (r.legs[0] || {}).fra, (r.legs[0] || {}).til, false)));
        innPaga.forEach(r => {
            const ri = returInfo(r);
            if (returVis === 'rute') {
                const osloBen = (r.legs || []).find(l => erVaartOmraade(l.til));
                jobber.push(leggRute(r, ri.fra, osloBen ? osloBen.til : 'Oslo', true));
                return;
            }
            jobber.push((async () => {
                ...
                markorer.push({ lngLat: lngLat, tip: '🚐 ' + (r.ressurs || '') + ' – ' + (stedFraAdr(ri.fra) || ''),
                    html: '<div style="position:relative;width:20px;height:20px"><div style="width:20px;height:20px;border-radius:50%;background:' + turFarge(r) + ';opacity:.9;border:2px solid #fff;box-shadow:0 0 3px #000"></div><div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:11px">🚐</div></div>' });
                ref[r.resId] = { center: lngLat };
            })());
        });
        await Promise.all(jobber);
        kartSiste = { ruterFC: ruterFC, punkterFC: punkterFC, markorer: markorer };
        kartRef = ref;
        applyKart();
    }
    ...
    async function velgOmraade(idx) {
        ...
        if (win && win.document && !win.__oaVisHook) {
            ...
            win.document.addEventListener('visibilitychange', () => { if (win && !win.closed && !win.document.hidden) tikk(); });
        }
