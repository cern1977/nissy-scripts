...
(function () {
    ...
    async function visSamkjorKart(par) {
        ...
        try {
            ...
            for (let i = 0; i < 3; i++) {
                const llA = P[beste[i]].ll, llB = P[beste[i + 1]].ll;
                if (!gyldigKoord(llA) || !gyldigKoord(llB)) continue;
                if (llA[0] === llB[0] && llA[1] === llB[1]) continue;  // samme punkt (f.eks. felles hentested)
                // Rute på KOORDINATER (rask, hopper over treg klinikk-geokoding som timet ut for hente-segmenter).
                let geo = null;
                ...
                const fallback = !geo;
                if (fallback) geo = [llA, llB];  // rett (stiplet) linje hvis ruting feiler — alltid synlig kobling
